...
import com.yuejian.meet.session.avchat.widgets.MultiSelectDialog;
...
//        }
    ...
    public void outGoingCalling(String account, final AVChatType callTypeEnum) {
        ...
        try {
            jsonObject.put("nickName", AppConfig.userEntity.getName());
            jsonObject.put("photo",AppConfig.userEntity.getPhoto());
        } catch (JSONException e) {
            e.printStackTrace();
        }
        notifyOption.extendMessage = jsonObject.toString();
//        notifyOption.extendMessage = "extra_data";
        notifyOption.webRTCCompat = webrtcCompat;
        notifyOption.apnsContent = "视频聊天请求";
...
//        notifyOption.forceKeepCalling = false;
        ...
        AVChatManager.getInstance().call2(account, callTypeEnum, notifyOption, new AVChatCallback<AVChatData>() {
            ...
            public void onSuccess(AVChatData data) {
                avChatData = data;
                AppConfig.getChatId=data.getChatId()+"";
...
//                }
            ...
            public void onFailed(int code) {
                ...
                }else if(code==ResponseCode.RES_IN_BLACK_LIST){
                    Toast.makeText(context, "你已被对方拉黑", Toast.LENGTH_SHORT).show();
                } else {
...
//                    }
    ...
    public void closeSessions(int exitCode) {
        ...
        isClosedCamera = AppConfig.UserSex.equals("1")?true:false;
    ...
    public void closeCamera() {
        if (!isClosedCamera) {
            ...
            Bus.getDefault().post("close_camera");
        } else {
            // 打开摄像头
            AVChatManager.getInstance().muteLocalVideo(false);
            isClosedCamera = false;
            avChatSurface.localVideoOn();
            Bus.getDefault().post("open_camera");
        }
...
//        avChatSurface.switchCamera();
    ...
    public void toggleCloseCamera(boolean isClosedCamera){
//        avChatSurface.toggleCloseSmallCamera(isClosedCamera);
    }
}
