...
public class NineGridImageView<T> extends ViewGroup {
    ...
    public NineGridImageView(Context context, AttributeSet attrs) {
        ...
        this.mImageSize = typedArray.getDimensionPixelSize(R.styleable.NineGridImageView_imgSize, -1);
        typedArray.recycle();
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (mImgDataList != null && mImgDataList.size() > 0) {
            ...
            if (mImgDataList.size() == 1) {
                totalWidth = mGridSize;
            } else {
                totalWidth = mGridSize * mColumnCount + mGap * (mColumnCount - 1);
            }
    ...
    private void layoutForNoSpanChildrenView(int childrenCount) {
        ...
        for (int i = 0; i < childrenCount; i++) {
            ...
            if (childrenCount == 2 && mSpanType == LEFTROWSPAN) {
    ...
    public void setImagesData(List<T> lists, int spanType, int[] gridParam) {
        if (lists == null || lists.isEmpty()) {
            this.setVisibility(GONE);
            return;
        } else {
