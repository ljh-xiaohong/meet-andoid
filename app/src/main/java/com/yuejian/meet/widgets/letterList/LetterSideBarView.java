...
public class LetterSideBarView extends View {
    ...
    public LetterSideBarView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mLazyRespond = typedArray.getBoolean(R.styleable.LetterSideBarView_sidebar_lazy_respond, false);
        mTextColor = typedArray.getColor(R.styleable.LetterSideBarView_sidebar_text_color, Color.GRAY);
        mMaxOffset = typedArray.getDimension(R.styleable.LetterSideBarView_sidebar_max_offset, dp2px(DEFAULT_MAX_OFFSET));
        mSideBarPosition = typedArray.getInt(R.styleable.LetterSideBarView_sidebar_position, POSITION_RIGHT);
        mTextAlignment = typedArray.getInt(R.styleable.LetterSideBarView_sidebar_text_alignment, TEXT_ALIGN_CENTER);
        typedArray.recycle();
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0, mIndexItemsLength = mIndexItems.length; i < mIndexItemsLength; i++) {
            ...
            if(i == mCurrentIndex){
                mPaint.setColor(context.getResources().getColor(R.color.dialog_message));
            }else {
                mPaint.setColor(context.getResources().getColor(R.color.black6));
            }
            mPaint.setTextSize(mTextSize);
            float baseLineX = 0f;
