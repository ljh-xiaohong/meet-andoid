...
public class MessageListPanelEx {
    ...
    boolean isSendMessage=false;//判断当天是否发过消息或 收到消息

    MessageFragment messageFragment;


    public MessageListPanelEx(MessageFragment messageFragment,Container container, View rootView, boolean recordOnly, boolean remote) {
        this(messageFragment,container, rootView, null, recordOnly, remote);
    }

    public MessageListPanelEx(MessageFragment messageFragment,Container container, View rootView, IMMessage anchor, boolean recordOnly, boolean remote) {
        this.messageFragment=messageFragment;
        this.container = container;
        this.rootView = rootView;
        this.recordOnly = recordOnly;
        this.remote = remote;
        NIMClient.getService(TeamService.class).queryTeamMember("","");
        init(anchor);
    }
...
//        adapter.setEmptyView(View.inflate(container.activity,R.layout.item_group_message_header,null));
//        messageListView.addView(View.inflate(container.activity,R.layout.item_group_message_header,null),0);
    }
    ...
    public void onIncomingMessage(List<IMMessage> messages) {
        IMMessage mMymessage=messages.get(0);
        if (mMymessage.getSessionType()==SessionTypeEnum.P2P&&mMymessage.getDirect()== MsgDirectionEnum.In){
            if (!PreferencesMessage.readBoolean(rootView.getContext(),PreferencesMessage.isChat+mMymessage.getFromAccount()))
                PreferencesMessage.write(rootView.getContext(),PreferencesMessage.isChat+mMymessage.getFromAccount(),true);//对方发消息过来保存可以自由聊天
        }
//        if (!isSendMessage&&messages.size()>0&&AppConfig.UserSex.equals("0")){///判断我是否为女号 并且今天没跟他聊过天
//            NimUserInfo userInfo= NimUserInfoCache.getInstance().getUserInfo(container.account);
//            Integer poSex=userInfo.getGenderEnum().getValue();
//            if (poSex==2){///判断对方是否为女号
//                IMMessage mMymessage=messages.get(0);
//                Long time=Utils.getLongDate();
//                if (mMymessage.getFromAccount().equals(AppConfig.CustomerId)){
//                    PreferencesMessage.put(rootView.getContext(),AppConfig.CustomerId+container.account+time,"2");
//                }else{
//                    PreferencesMessage.put(rootView.getContext(),AppConfig.CustomerId+container.account+time,"1");
//                }
//            }
//            isSendMessage=true;
//        }
        ...
        for (IMMessage message : messages) {
            if (isMyMessage(message)) {
                if (message.getAttachment() instanceof MyTiptAttachment){
                    ...
                    if (!StringUtil.isEmpty(envelopeEntity.getOp_customer_id()))
                        if (!envelopeEntity.getOp_customer_id().equals(AppConfig.CustomerId) && !envelopeEntity.getCustomer_id().equals(AppConfig.CustomerId)){continue;}
                }
                items.add(message);
    ...
    private class MessageLoader implements BaseFetchLoadAdapter.RequestLoadMoreListener, BaseFetchLoadAdapter.RequestFetchMoreListener {
        ...
        private RequestCallback<List<IMMessage>> callback = new RequestCallbackWrapper<List<IMMessage>>() {
            ...
            public void onResult(int code, List<IMMessage> messages, Throwable exception) {
                ...
                ///判断拿历史消息判断当天是谁先发的
//                Long messageTime= Utils.getLongDate();
//                if (PreferencesMessage.get(rootView.getContext(),AppConfig.CustomerId+container.account+messageTime,"").equals("")&&AppConfig.UserSex.equals("0")){///判断今天有没有跟这个女号聊过天并且我也是女号
//                    NimUserInfo userInfo=NimUserInfoCache.getInstance().getUserInfo(container.account);
//                    if (userInfo!=null){
//                        Integer poSex=userInfo.getGenderEnum().getValue();
//                        if (messages!=null&&messages.size()>0&&poSex==2){
//                            for(IMMessage readMessage:messages){
//                                if (readMessage.getTime()>messageTime){
//                                    if (readMessage.getFromAccount().equals(AppConfig.CustomerId)){///第一条我发的设置收费
//                                        PreferencesMessage.put(rootView.getContext(),AppConfig.CustomerId+container.account+messageTime,"2");
//                                    }else{///第一条她发的设置我不收费
//                                        PreferencesMessage.put(rootView.getContext(),AppConfig.CustomerId+container.account+messageTime,"1");
//                                    }
//                                    isSendMessage=true;
//                                    break;
//                                }
...
//                }
        ...
        private void onMessageLoaded(final List<IMMessage> messages) {
            ...
            if (!PreferencesMessage.readBoolean(container.activity,PreferencesMessage.isChat+container.account)){
                for (IMMessage  mm:messages){
                    if (mm.getDirect()==MsgDirectionEnum.In){
                        PreferencesMessage.write(container.activity,PreferencesMessage.isChat+container.account,true);//对方有发过消息则可以自由聊天
                        break;
                    }
                }
            }
    ...
    private class MsgItemEventListener implements MsgAdapter.ViewHolderEventListener {
        ...
        @Override
        public void onViewUserPotoLoneClick(TeamMember member) {
            messageFragment.putPrivate(member);
        }
